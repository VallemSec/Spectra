...
def safe_eval(expr, variables=None) -> bool:
    ...
    It is known that `eval` is unsafe, but the use of it here is justified since there will
    be no unauthorized input.
    :param expr: The boolean expression to be evaluated
    :param variables: A dictionary with the variable name as the key and the value as value
    ...
    try:
        ...
        if not isinstance(result, bool):
            raise ValueError(f"Evaluated statement is not a boolean")
        return result
